...
class JobParser(private val htmlSource: HtmlSource? = null) {
    ...
    private suspend fun fetch(url: String): Document = coroutineScope {
        ...
        val deferreds = listOf(mobileUa, desktopUa).map { ua ->
            async(Dispatchers.IO) {
                runCatching { connect(url, ua) }
            }
        }
        ...
        // Pick first success
        val success = results.firstOrNull { it.isSuccess }?.getOrThrow()
        if (success != null) return@coroutineScope success

        // If all failed, analyze the first error to throw a meaningful exception
        val firstError = results.first().exceptionOrNull()
        if (firstError is HttpStatusException) {
            val code = firstError.statusCode
            if (code == 403 || code == 429 || code == 999 || code == 401 || code == 503) {
                throw BlockedException(code)
            }
            throw IOException("The site returned an error (HTTP $code). It may be blocking automated requests.")
        }
        
        throw firstError ?: IOException("The site blocked automated access. Try pasting the link again.")
    }
    ...
    private fun connect(url: String, ua: String): Document =
        Jsoup.connect(url)
            ...
            .timeout(12_000) // Reduced from 25s
