...
private fun QuestionItem(
    ...
    val cleanInitial = if (initialAnswer.trim().length <= 1) "" else initialAnswer
    var text by remember(question.id) { mutableStateOf(cleanInitial) }

    LaunchedEffect(initialAnswer) {
        val updated = if (initialAnswer.trim().length <= 1) "" else initialAnswer
        if (text != updated && text.isBlank()) {
            text = updated
        }
    }
